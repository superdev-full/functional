...
function check_loggedin($con, $redirect_file = maverEnvironment) {
...
function send_activation_email($email, $code) {
	...
	$email_template = str_replace('%link%', $activate_link, file_get_contents(EMAILS_PATH . 'activation-email-template.html'));
	// Send email to user
	mail($email, $subject, $email_template, $headers);
}

// Send new account email function
function send_new_account_email($email) {
	// Email Subject
	$subject = 'New Account Creation';
	// Email Headers
	$headers = 'From: ' . mail_from . "\r\n" . 'Reply-To: ' . mail_from . "\r\n" . 'Return-Path: ' . mail_from . "\r\n" . 'X-Mailer: PHP/' . phpversion() . "\r\n" . 'MIME-Version: 1.0' . "\r\n" . 'Content-Type: text/html; charset=UTF-8' . "\r\n";
	// Read the template contents
	$email_template = file_get_contents(EMAILS_PATH . 'new-account-email-template.html');
	// Send email to user
	mail($email, $subject, $email_template, $headers);
}
?>
