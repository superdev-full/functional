...
if (isset($_GET['id'])) {
    ...
    if (!empty($_GET['action']) && !empty($_GET['owner_id']) && $_GET['action']=="bid" && !empty($_GET['bid_amount']) && is_numeric($_GET['bid_amount'])  ) {
        ...
        $bid_action = $bids_ctrl->set_user_bid( $assignment["id"], $owner_id , $bid_amount, $_SESSION["id"] );
    ...
    if (!empty($_GET['action']) && !empty($_GET['owner_id']) && $_GET['action']=="retract") {
        ...
        $owner_id = $_GET['owner_id'];
        $bid_action = $bids_ctrl->retract_user_bid( $assignment["id"], $owner_id , $_SESSION["id"] );
        if($bid_action["status"] == 1){
    ...
    if (!empty($_POST['solution_action']) && !empty($_GET['owner_id']) && $_POST['solution_action']=="update_solution") {
        ...
        $action_status = $bids_ctrl->update_solution( $assignment_id, $bid_id,  $owner_id , $solution, $_SESSION["id"] );
    ...
    if (!empty($_POST['solution_action']) && !empty($_GET['owner_id']) && $_POST['solution_action']=="finalise_solution") {
        ...
        $action_status = $bids_ctrl->finalise_solution( $assignment_id, $bid_id, $owner_id , $solution, $_SESSION["id"] );
        if($action_status["status"] == 1){
    ...
    <table>
        ...
        <tr>
            ...
            <td valign="top" style="padding:5px;background-color:#efefef;">
                    <?php
                    $bid_info = $bids_ctrl->can_user_bid( $assignment["id"], $_SESSION["id"] ,$_SESSION["role"] );
                    if( $bid_info["status"]  == 1){
                        ?>
                        <div id="candidates" style="text-align:center;background-color:#fff;border:1px solid #ececec;padding:20px;">
                        <form action="assignment.php" method="get" onsubmit="return confirm('Are you sure you want to place a Bid on this Assignment?');">
                            <input type="hidden" name="id" value="<?php echo $_GET["id"]?>" >
                            <input type="hidden" name="owner_id" value="<?php echo $assignment["user_id"];?>" >
                        ...
                        <a href="assignment.php?id=<?php echo $_GET["id"]?>&owner_id=<?php echo $assignment["user_id"];?>&action=retract" onclick="return confirm('Are you sure you want to RETRACT a Bid on this Assignment?');">Retract your Bid!</a>
                        <?php } ?>
                    </div>
                        <?php
                    }else{
            ...
            <td valign="top">
                ...
                if( ( $bid_info["selected"] && $bid_info["paid"] ) && !$bid_info["finalised"] ){
                    ...
                    ?>
                    <form id="frmSolution" action="assignment.php?id=<?php echo $_GET["id"]?>&bid_id=<?php echo $solution["id"]?>&owner_id=<?php echo $assignment["user_id"];?>" method="post" onsubmit="return confirm('Are you sure?');"  enctype="multipart/form-data">
