...
class BidsController{
	 ...
	 * @return $view , HTML
	 */			
	function get_candidates_list( $question_id, $user_id , $role = "Member"){

		$sql = "SELECT a.id,a.applied_at,a.selected_at,a.rejected_at,a.paid_at,a.answered_at,a.bid_amount,a.solution,a.attachment,b.id as user_id,b.username,b.avatar FROM bids as a INNER JOIN accounts as b ON b.id=a.candidate_id WHERE (a.question_id = ".$question_id." ) ORDER BY a.applied_at DESC";
        // var_dump($sql);
			...
			while($candidate = $result->fetch_assoc()) {
				...
				// Role is member
					...
					if($candidate["selected_at"]!=null){
						...
						$view_tmp_actions = str_replace("[QUESTION_ID]", $question_id, $template_member_actions);
	 ...
	 */			
	function get_selected_bid( $question_id ,$role = "Member" ){
		$winner = "N/A";
		$solution = "N/A";

		if( $role == "Tutor" || $role == "Admin" || $role == "Member"){
			// check if the user has alread bid
			$sql = "SELECT a.id,a.notes,a.solution,a.attachment,a.applied_at,a.selected_at,a.rejected_at,b.id as user_id,b.username,b.avatar FROM bids as a INNER JOIN accounts as b ON b.id=a.candidate_id  WHERE a.question_id=" . $question_id . " AND a.selected_at IS NOT NULL";
	...
	function get_bid( $id ){
		...
		return [ "status" =>  $status, "message" => $message, "question_id" => $question_id, "bid_amount" => $bid_amount, "selected" => $selected, "rejected" => $rejected, "paid" => $paid, "finalised" => $finalised ];
	}
	 ...
	 * @return $status , boolean
	 */			
	function can_user_bid( $question_id, $user_id ,$role ){
		$status 	= -1;
		$message 	= "You cannot bid for this question";
		$bid_amount = "";
		$selected 	= false;
		$rejected 	= false;
		$paid 		= false;
		...
		if( $role == "Tutor" ){
			...
			$sql = "SELECT * FROM `bids` WHERE question_id=" . $question_id . " AND candidate_id=" . $user_id;
			$result = $this->model->db_query_select_row( $sql  );
			if($result){
				// user already bid
				$status = 0;
				$message 	= "You have already placed your Bid for this question.";
				$bid_amount = $result["bid_amount"];
				if($result["selected_at"] != NULL){
					$selected = true;
				}
			...
			$message = "You cannot bid for this question. Wrong member role.";
		}
	 ...
	 * @return $status , boolean
	 */			
	function decline_bid( $question_id, $bid_id , $owner_id ){
		$status = -1;
		...
		$sql = "UPDATE `bids` set  `rejected_at`='".$date."'  WHERE `question_id`=".$question_id." AND `id`=".$bid_id." AND `owner_id`=".$owner_id . ";";
		$result  = $this->model->db_query_update( $sql );
	 ...
	 * @return $status , boolean
	 */			
	function accept_bid( $question_id, $bid_id , $owner_id ){
		...
		if($result && mysqli_num_rows($result)>0){
			while($candidate = $result->fetch_assoc()) {
				$this->decline_bid($question_id, $candidate["id"], $owner_id);
			}
		}
		...
		$sql = "UPDATE `bids` set  `selected_at`='".$date."'  WHERE `question_id`=".$question_id." AND `id`=".$bid_id." AND `owner_id`=".$owner_id . ";";
		$result  = $this->model->db_query_update( $sql );
	 ...
	 * @return $status , boolean
	 */			
	function set_user_bid( $question_id, $owner_id, $bid_amount, $candidate_id ){
		$status = -1;
		...
		if($this->can_user_bid( $question_id, $candidate_id ,"Tutor" )[ "status" ] == 1){

			$sql = "INSERT INTO `bids` (`question_id`,`owner_id`, `candidate_id`, `bid_amount`, `notes`, `applied_at`, `retracked_at`, `selected_at` , `rejected_at` , `updated_at`) VALUES (?, ?, ?, ? ,?, ?, ?, ?, ?, ?);";
			$result  = $this->db_query_insert( $sql , $params );

			if($result){
				// user succesffuly bid
				$status = 1;
				$message = "You have successfully placed your bid for this question!";
			}else{
				$status = -1;
	 ...
	 * @return $status , boolean
	 */			
	function retract_user_bid( $question_id, $owner_id, $candidate_id ){
		$status = -1;
		...
		$sql = "DELETE FROM `bids` WHERE `question_id` = ".$question_id." AND `owner_id` = ".$owner_id." AND  `candidate_id`=" . $candidate_id;
		$result  = $this->model->db_query_update( $sql );

		if($result){

			$status = 1;
			$message = "You have successfully retracted your bid for this question!";
		}else{
			$status = -1;
	 ...
	 * @return $status , boolean
	 */			
	function update_solution( $question_id,$bid_id , $owner_id, $solution ){
		$status = -1;
		...
		$sql = "UPDATE `bids` set `solution`='". $solution ."',`attachment`='". $attachment_url ."',updated_at='".$date."' WHERE `id` = ".$bid_id." AND `question_id` = ".$question_id." AND `owner_id` = ".$owner_id;
		$result  = $this->model->db_query_update( $sql );

		if($result){

			$status = 1;
			$message = "You have successfully updated your solution for this question!";
		}else{
			$status = -1;
	 ...
	 * @return $status , boolean
	 */			
	function finalise_solution( $question_id ,$bid_id ,$owner_id, $solution ){
		$status = -1;
		...
		$sql = "UPDATE `bids` set `solution`='". $solution ."',`attachment`='". $attachment_url ."',updated_at='".$date."',answered_at='".$date."' WHERE `id` = ".$bid_id." AND `question_id` = ".$question_id." AND `owner_id` = ".$owner_id;
		$result  = $this->model->db_query_update( $sql );

		if($result){

			$status = 1;
			$message = "You have successfully finalised your solution for this question!";
		}else{
			$status = -1;
